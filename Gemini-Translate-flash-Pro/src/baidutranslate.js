// Baidutranslate.js (Phiên bản Thư viện Dữ liệu - Chỉ trả về chuỗi thô)

var languageMap = {
    "auto": "auto", "zh": "zh", "en": "en", "vi": "vie"
};

/**
 * Dịch văn bản bằng Baidu. Khi thành công, trả về chuỗi kết quả. Khi thất bại, trả về null.
 * @returns {string|null}
 */
function baiduTranslateContent(text, from, to, retryCount) {
    ...
    var data = {
        query: text, from: from, to: to, reference: "", corpusIds: [],
        needPhonetic: true, domain: "common", milliTimestamp: Date.now()
    };

    var response = fetch("https://fanyi.baidu.com/ait/text/translate", {
        method: 'POST',
        headers: { 'Referer': 'https://fanyi.baidu.com/', 'Content-Type': 'application/json' },
                ...
                try {
                    ...
                    if (obj && obj.data && obj.data.list) {
                        ...
                        for (var j = 0; j < rData.length; j++) {
                            var item = rData[j];
                            if (item && item.dst) { trans += item.dst + "\n"; }
                        }
                    }
                } catch (e) {
                    console.log("Lỗi parse JSON trong Baidu: " + e.toString());
                }
            }
        }
        
        // --- THAY ĐỔI DUY NHẤT VÀ QUAN TRỌNG NHẤT ---
        // Chỉ trả về chuỗi văn bản thô, không còn Response.success()
        if (trans.trim() !== "") {
            return trans.trim();
        }
        // Nếu không có nội dung, sẽ rơi xuống và thử lại
    }
