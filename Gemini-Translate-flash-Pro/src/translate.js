...
function execute(text, from, to) {
    ...
    if (text.length < 200) {
        ...
        // --- LOGIC MỚI: CHUẨN HÓA MÃ NGÔN NGỮ ĐÍCH CHO BAIDU ---
        var baiduToLang = to;
        if (to === 'vi_sac' || to === 'vi_vietlai' || to === 'vi_NameEng') {
            baiduToLang = 'vi'; // Tất cả các biến thể tiếng Việt đều được dịch sang 'vi'
        }
        // Các mã khác như 'en', 'zh' sẽ được giữ nguyên và xử lý bởi languageMap trong Baidu
        ...
        if (baiduResult) {
            ...
            return Response.error("Lỗi khi dịch bằng Baidu Translate.");
        }
    }
    ...
    // BƯỚC 2A: TIỀN XỬ LÝ (QUYẾT ĐỊNH PHIÊN ÂM)
    ...
    // BƯỚC 2B: CHIA NHỎ VĂN BẢN ĐÃ XỬ LÝ
    var textChunks = [];
    var CHUNK_SIZE = 5000;
    ...
    // BƯỚC 2C: GỬI ĐẾN GEMINI VÀ GOM KẾT QUẢ
    var finalParts = [];
    ...
    // BƯỚC 2D: CHUẨN HÓA KẾT QUẢ ĐỂ HIỂN THỊ
